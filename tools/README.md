...
| **`positions.py`** | **★ 實際持倉：股數、每股成本、已領股利**（守則第 20 節） | 券商 App，**只在買賣後才改** |
...
- **持倉只放 `inputs/positions.py`**：即時頁與每日報告共用同一份，買賣後改這一個檔案就好。
  減碼／出場張數是把 `zones.py` 的區間規則套到實際張數的**算術結果**（分母是既有持倉）；
  **加碼張數一律不算**，只報區間端點與每 1 張金額（分母是可動用資金，程式不知道）。守則第 20 節。
